...
CLADES_ORDER = rev(c('Canonical Microsporidia', 'Metchnikovellids',
...
main <- function() {
  ...
  yeast_domain_archs <- format_yeast_domain_archs(
    read_delim('../../data/yeast_domains_resolved.txt')
  )
  ...
  # load in single copy orthogroups for supp fig 1
  orthogroups <- read_csv('../../results/single_copy_orthogroups.csv') %>%
    filter() %>%
    rowwise() %>%
    mutate(clade = clades[[species]],
           essential = ifelse(ref_ortholog %in% essential_yeast_genes,
                              'Essential',
                              'Non-essential'))
  
  lost_doms <- aligned_domain_archs %>%
    filter(!is.na(lost_doms)) %>%
    rename(gene = ref_ortholog) %>%
    left_join(ptc_data, by = 'gene') %>%
    rename(ref_ortholog = gene) %>%
    filter(!is.na(domain_ptc_annotations)) %>%
    mutate(lost_dom_annotations = get_lost_dom_annotations(aligned_domain_archs,
                                                           domain_ptc_annotations),
           lost_essential = lost_essential_domain(lost_dom_annotations),
           lost_dispensible = lost_dispensible_domain(lost_dom_annotations),
           lost_ambiguous = lost_ambiguous_domain(lost_dom_annotations))
  ...
  plot_number_orthologs(orthogroups)
  plot_lost_dom_types_percents(lost_doms)
...
lost_essential_domain <- Vectorize(function(lost_dom_annotations) {
  return(as.integer(str_detect(lost_dom_annotations, 'Essential')))
})


lost_dispensible_domain <- Vectorize(function(lost_dom_annotations) {
  return(as.integer(str_detect(lost_dom_annotations, 'Dispensable')))
})


lost_ambiguous_domain <- Vectorize(function(lost_dom_annotations) {
  return(as.integer(str_detect(lost_dom_annotations, 'Ambiguous')))
})
...
plot_lost_dom_types <- function(lost_doms) {
  ...
  # ---------------------------------------------------------------------------
  plot_df <- lost_doms %>%
    filter(!is.na(lost_doms), short_enough_for_domain_loss, !exclude_species) %>%
    group_by(species, clade) %>%
    summarise(n_lost_essential = sum(lost_essential),
              n_lost_dispensible = sum(lost_dispensible),
              n_lost_ambiguous = sum(lost_ambiguous)) %>%
    group_by(clade) %>%
    summarise("Essential domain lost" = mean(n_lost_essential),
              "Dispensible domain lost" = mean(n_lost_dispensible)) %>%
    pivot_longer(cols = c("Essential domain lost", "Dispensible domain lost"),
                 names_to = "Loss type",
                 values_to = "avg")
  
  ggplot(plot_df, aes(x = clade, y = avg, fill = `Loss type`)) +
    geom_bar(stat = "identity", position = "dodge") +
    labs(x = "Clade",
         y = "Average # of orthologs per species losing at least one such domain") +
    scale_x_discrete(limits = CLADES_ORDER) +
    # coord_flip() +
    theme_bw() +
  ...
  # ggplot(filter(lost_doms, short_enough_for_domain_loss, !exclude_species),
  #        aes(x = factor(clade, level = CLADES_ORDER))) +
...
plot_number_orthologs <- function(orthogroups) {
  orthogroups %>%
    group_by(species, essential) %>%
    summarise(n = n()) %>%
    ggplot(aes(x = species, y = n, fill = essential)) +
    geom_bar(stat = 'identity') +
    scale_fill_manual('Essential in yeast',
...
plot_lost_dom_types_percents <- function(lost_doms) {
  # ---------------------------------------------------------------------------
  # ---------------------------------------------------------------------------
  ggplot(filter(lost_doms, short_enough_for_domain_loss, !exclude_species), aes(x = clade)) +
    geom_bar(aes(fill = lost_dom_annotations), position='fill',
             show.legend = FALSE) +
    coord_flip() +
    theme_bw() +
    labs(y = '% of lost domains in orthologs to yeast') +
    scale_fill_manual('Lost domain classification',
                      values = c('#F7DC6F', '#2ECC71', '#E74C3C')) +
    scale_x_discrete(limits = CLADES_ORDER) +
    theme(axis.title.y = element_blank(),
