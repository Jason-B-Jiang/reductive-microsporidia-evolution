...
REF_SPECIES_NAME = 'Saccharomyces cerevisiae'
...
get_ortholog_domain_arch_conservation <- function(aligned_domain_archs) {
  ...
  ortholog_domain_arch_conservation <- aligned_domain_archs %>%
    ...
    mutate(domain_arch_conservation = get_DA_conservation_class(lost_doms,
                                                                gained_doms,
                                                                swapped_doms)) %>%
    select(species, ref_ortholog, domain_arch_conservation)
...
fill_missing_orthos <- function(ortholog_domain_arch_conservation) {
  # ----------------------------------------------------------------------------
  # For each species, add rows for reference species genes that the species
  # doesn't have orthologs to, annotating them as "Ortholog not conserved" in the
  # ortholog_domain_arch_conservation dataframe.
  #
  # Args:
  #   ortholog_domain_arch_conservation: dataframe of species + reference ortholog
  #   domain architecture conservations, with columns for species name, reference
  #   gene and ortholog domain arch conservation with that reference gene in the
  #   species.
  #
  # ----------------------------------------------------------------------------
  # get all unique orthologs that microsporidia share with reference species
  ref_orthos <- unique(ortholog_domain_arch_conservation$ref_ortholog)
  ...
  for (i in 1 : length(sp_DA_conservation)) {
    ...
    missing_orthos <- setdiff(ref_orthos, sp_DA_conservation[[i]]$ref_ortholog)
...
create_ortholog_tileplot <- function(ortholog_domain_arch_conservation) {
  sp_order <- c('A_alge', 'A_locu', 'E_aedi', 'E_bien', 'E_brev', 'E_canc',
                ...
                'N_homo', 'N_iron', 'N_majo', 'N_mino', 'N_botr', 'N_pari',
  ...
  agnes_cluster_order <- get_ref_genes_clusters(ortholog_domain_arch_conservation)
  ortholog_domain_arch_conservation$ref_ortholog <-
    factor(x = ortholog_domain_arch_conservation$ref_ortholog,
           levels = ortholog_domain_arch_conservation$ref_ortholog[agnes_cluster_order],
           ordered = T)
  
  ortholog_domain_arch_conservation <- ortholog_domain_arch_conservation %>%
    mutate(species = ifelse(species == "N_okwa", "N_botr", species))
  
  tileplot <- ggplot(ortholog_domain_arch_conservation, aes(x = ref_ortholog,
                                                            y = species)) +
...
get_ref_genes_clusters <- function(ortholog_domain_arch_conservation) {
  # ----------------------------------------------------------------------------
  # ----------------------------------------------------------------------------
  # 'widen' the dataframe by making a column for each species, so we have a
  # table of reference species genes and their domain arch conservation with
  # single-copy orthologs from each species
