# -----------------------------------------------------------------------------
#
# Plot t-SNE of species with lost domains to reference species
#
...
#
# Goal: Use k-medoids clustering to cluster species by their domain losses
#       w.r.t to their reference species orthologs
#
...
PERPLEXITY <- 10  # perplexity value for t-SNE plots, adjust to your liking
...
main <- function() {
  ...
  out <- '../../results/fig_5/lost_doms_tsne.svg'
  ...
  return(
    fill_missing_clans(
      aligned_domain_archs %>%
        # filter(!is.na(lost_doms), !excluded_species, !is_outgroup) %>%
        filter(!is.na(lost_doms)) %>%
        select(species, lost_doms) %>%
        rename(clans = lost_doms) %>%
        separate_rows(clans, sep = '; ') %>%
        distinct(.keep_all = T) %>%
        mutate(clan_lost = 1)
    ) %>%
...
cluster_microsp_by_lost_clans <- function(species_clan_losses,
  ...
  sil_width <- c(NA)
  for (i in min(2, nrow(species_clan_losses) - 1) : (nrow(species_clan_losses) - 1)) {
...
plot_lost_dom_tsne <- function(k_medoids_clusters, gower_dist,
                               species_clan_losses, species_clades,
                               out) {
  # ---------------------------------------------------------------------------
  # ---------------------------------------------------------------------------
  ...
  tsne_data <- tsne_obj$Y %>%
    ...
    arrange(cluster) %>%
    rowwise() %>%
    mutate(clade = ifelse(is_null(species_clades[[species]]),
                          'Outgroup',
